...
def cnn_train(model, data, epochs, optimizer, scheduler, device='cuda', tensor_board_path='', models_path='', args=None):
    ...
    for epoch in range(1, epochs):      
        ...
        for x, y, b_mask, sensitive_group in tqdm(train_loader): #, mask
            ...
            target_snnl = t_id_snnl = t_same_id_snnl = s_id_snnl = sensitive_snnl = 0
...
if __name__ == '__main__':
    ...
    model = VGG11_AttEN(class_num=args.class_num)
    model.to(device)    
    ...
    cnn_test(model, dataset.test_loader, 'VGG', device, training_title)