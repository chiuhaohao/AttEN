        ...
        for x, y, b_mask, sensitive_group in tqdm(train_loader): #, mask
            ...
            if (sam_prompt):
                for idx, residual_map in enumerate(attention_map_list):
                    ...
                    new_prompt_msk[:, 0, :, :] = channel_mean # 0.7
                    residual_map_copy = residual_map.clone()
                    # residual_map_copy *= new_prompt_msk
            ...
            for idx, residual_feature in enumerate(attention_feature):
                for label in range(args.class_num):
                    mask = b_y == label
                    t_id_snnl = SNNL(residual_feature[mask], b_y[mask])
                    # id_snnl = SNNL(residual_feature[mask], b_y[mask]).detach().cpu().numpy()
                    if (torch.isnan(t_id_snnl) or torch.isinf(t_id_snnl)):
                        continue
                    target_snnl += t_id_snnl # target_snnl need to be minimized
            for idx, residual_feature in enumerate(attention_feature):
            ...
            # print(sensitive_snnl)
            _, preds = torch.max(output, 1) 
            
            criterion = af.get_loss_criterion('')
            loss = criterion(output, b_y)
            # loss = loss + alpha*( 10*target_snnl - sensitive_snnl)
            # print(loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=256,
                    help='')
    parser.add_argument('--dataset', type=str, default='fitzpatrick17k_mask',
                    help='')
    ...
    model = Resnet18_CBAMAS(class_num=args.class_num)
    # model = torch.load('/home/jinghao/Fairness/SAM/networks/121/20230711_train_resnet18_isic_cbam/80.pth')
    model.to(device)
    ...
    # dataset = ISIC2019(batch_size=args.batch_size)
    # dataset = ISIC2018(batch_size=args.batch_size)
    # print(model)
